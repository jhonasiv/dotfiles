...
- `gh_repo.js` - exports the tool family
...
2. `gh_repo_session` with `operation: "configure"` sets `pushes_remaining`, `review_rounds_remaining`, `stop_when_approved`, `stop_when_checks_pass`, and optional `notes`.
3. `gh_repo_workflow` with `operation: "status"` inspects the current PR, unresolved threads, checks, budget, and suggested next actions.
...
2. **PR**: Ensures a PR exists for the current branch/bookmark (creates one if needed via `gh pr create`).
...
| `needs_pr` | No PR found for current branch |
...
1. Their latest GitHub review state is `APPROVED` (and newer than the last push), or
2. They posted an approval-like comment (`lgtm`, `approved`, `looks good`, `no more changes`, `ship it`) after the last push.
